...
import { FaEdit, FaTrash, FaImage, FaPlus } from 'react-icons/fa'; // Added FaPlus for consistency
import { IoMdClose } from 'react-icons/io';
import { PropagateLoader } from 'react-spinners';
import { overrideStyle } from '../../utils/utils';
import { serviceCategoryAdd, messageClear, get_serviceCategory, updateServiceCategory, deleteServiceCategory } from '../../store/Reducers/serviceCategoryReducer';
...
const AddServiceCategory = () => {
    ...
    // Add loadingCategories ONLY if your reducer provides it
    const { loader, successMessage, errorMessage, serviceCategories = [], totalItems = 0 /* , loadingCategories */ } = useSelector(state => state.serviceCategory);

    // --- STATE (Unchanged from previous working version) ---
    ...
    // --- EVENT HANDLERS (Unchanged Logic, except handleDelete) ---
    const imageHandle = (e) => {
    ...
    const addOrUpdateCategory = (e) => {
        ...
        if (isEdit) {
            dispatch(updateServiceCategory({ id: editId, ...state }));
        } else {
            dispatch(serviceCategoryAdd(state));
        }
    };

     const handleEdit = (serviceCategory) => {
        setState({ name: serviceCategory.name, image: serviceCategory.image });
        setImage(serviceCategory.image);
    ...
    const handleDelete = (id) => {
        Swal.fire({
            title: 'Are you sure?',
            text: "You won't be able to revert this! Deleting this service category might also affect associated services.", // Updated text
            icon: 'warning',
    ...
    // --- USEEFFECT HOOKS (Unchanged Logic - Adjusted success message check) ---
    useEffect(() => {
        if (successMessage) {
    ...
    useEffect(() => {
        ...
        dispatch(get_serviceCategory(obj));
    ...
    return (
        // Outer container and background
        <div className="px-4 sm:px-6 lg:px-8 py-8 min-h-screen bg-gradient-to-br from-[#111827] to-[#1a1a2e] text-gray-300 relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-full mx-auto">
                 ...
                 <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
                    ...
                    <button
                        onClick={openAddForm}
                        className="lg:hidden bg-gradient-to-br from-cyan-500 to-purple-500 hover:shadow-[0_0_15px_rgba(0,255,255,0.5)] text-white font-medium px-4 py-2 rounded-lg shadow-md transition-all duration-300 text-sm flex items-center justify-center gap-2" // Added flex display
                    >
                        <FaPlus size={14} /> Add New Category {/* Added Icon */}
                    </button>
                 </div>
                ...
                <div className="flex flex-col lg:flex-row w-full gap-6 lg:gap-8">
                        ...
                        <div className="w-full p-5 sm:p-6 rounded-xl bg-[#252A40]/70 backdrop-blur-md border border-gray-700/80 shadow-lg">
                             ...
                             <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                                ...
                                <div className="flex items-center gap-3">
                                    <label htmlFor="tableParPage" className="text-sm text-gray-400 whitespace-nowrap">Show:</label>
                                    <select
                                        id="tableParPage"
                                ...
                                <table className="w-full text-sm text-left text-gray-300">
                                    ...
                                    <tbody className="divide-y divide-gray-700/50">
                                         ...
                                         {serviceCategories && serviceCategories.length > 0 ? (
                                                    ...
                                                    <td className="py-3 px-4 sm:px-6">
                                                        <div className="flex justify-center items-center gap-2 sm:gap-3">
                                                            <button
                                                                ...
                                                                className="p-2 text-sky-400 hover:text-sky-300 bg-sky-500/10 hover:bg-sky-500/20 rounded-md transition-all duration-300"
                                                                aria-label={`Edit ${d.name}`}
                                                            > <FaEdit size={14} /> </button>
                                                             {/* handleDelete now uses SweetAlert */}
                                                            <button
                                                                onClick={() => handleDelete(d._id)}
                                                                className="p-2 text-rose-400 hover:text-rose-300 bg-rose-500/10 hover:bg-rose-500/20 rounded-md transition-all duration-300"
                                            ...
                                            <tr>
                                                <td colSpan="4" className="py-8 px-4 sm:px-6 text-center text-gray-500 text-sm">
                                                    No service categories found{searchValue ? ` matching "${searchValue}"` : ''}.
                                                </td>
                                            </tr>
                                        )}
                                    </tbody>
                            ...
                            { totalItems > 0 && totalItems > parPage && (
                                <div className="w-full flex justify-center sm:justify-end mt-6">
                                    <Pagination
                                        pageNumber={currentPage} setPageNumber={setCurrentPage}
                                        totalItem={totalItems} parPage={parPage} showItem={3}
                                    />
                                </div>
                            )}
                        </div>
                    ...
                    {/* Form Section */}
                    <div
                        key={editId || 'add'} // Add key for potential form reset on edit/add switch
                        className={`fixed lg:static top-0 right-0 w-full max-w-md lg:w-5/12 h-full lg:h-auto transform transition-transform duration-300 ease-in-out z-[10000] ${
                            show ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'
                        } bg-[#252A40]/95 lg:bg-[#252A40]/70 backdrop-blur-md border-l border-gray-700/80 shadow-lg lg:rounded-xl lg:border lg:sticky lg:top-8`}
                    >
                        <div className="w-full h-full overflow-y-auto lg:overflow-y-visible px-5 sm:px-6 py-6">
                             {/* Form Header */}
                             ...
                             <form onSubmit={addOrUpdateCategory}>
                                <div className="flex flex-col gap-5">
                                    {/* Name Input */}
                                     <div>
                                        <label htmlFor="name" className="block text-sm font-medium text-gray-300 mb-1.5"> Category Name <span className="text-rose-500">*</span> </label>
                                        <input
                                             value={state.name} onChange={(e) => setState({ ...state, name: e.target.value })}
                                             className="w-full px-4 py-2.5 border rounded-lg outline-none transition-all duration-300 text-sm bg-[#303650]/80 border-gray-600 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-gray-200 placeholder-gray-400"
                                             type="text" id="name" placeholder="Enter category name" required
                                         />
                                    </div>
                                    ...
                                    <div>
                                         ...
                                         <label className={`flex justify-center items-center flex-col h-48 w-full cursor-pointer border-2 border-dashed rounded-lg transition-all duration-300 ${ imageShow ? 'border-cyan-500/50 p-1 bg-[#303650]/30' : 'border-gray-600 hover:border-purple-500 bg-[#303650]/30' }`} htmlFor="image">
                                             {imageShow ? (
                                                ...
                                                <div className="text-center p-4">
                                                    ...
                                                    <p className="text-xs text-gray-500 mt-1">PNG, JPG, WEBP, GIF (Max 2MB)</p>
                                                </div>
                                            )}
                                        </label>
                                        <input
                                            onChange={imageHandle} className="hidden" type="file"
                                            name="image" id="image" accept="image/png, image/jpeg, image/webp, image/gif"
                                        />
                                        {imageShow && (
                                            <button type="button" onClick={() => { setImage(''); setState(s => ({...s, image: ''})); const fi = document.getElementById('image'); if(fi) fi.value = null; }} className="text-xs text-rose-400 hover:text-rose-300 mt-2 underline" >
                                                Remove image
                                            </button>
                                         )}
                                    </div>
                                    ...
                                    <div className="mt-4 sm:mt-6">
                                        <button type="submit" disabled={loader}
                                            className={`w-full flex justify-center items-center bg-gradient-to-br from-cyan-500 to-purple-600 text-white font-semibold py-3 rounded-lg shadow-md transition-all duration-300 text-sm sm:text-base ${
                                                loader ? 'opacity-70 cursor-not-allowed from-gray-500 to-gray-600' : 'hover:from-cyan-600 hover:to-purple-700 hover:shadow-lg hover:shadow-cyan-500/30'
                                            }`} >
                                            {loader ? ( <PropagateLoader color="#ffffff" cssOverride={overrideStyle} size={10}/> ) : isEdit ? ( 'Update Service Category' ) : ( 'Add Service Category' )}
                                        </button>
                                    </div>
                                </div>
                            </form>
                    ...
                    {/* Mobile Overlay */}
                    {show && (
                        <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm z-[9999] lg:hidden" onClick={resetAndCloseForm} />
                    )}
                </div>
            </div>
