...
const SellerRequest = () => {
    ...
    const { sellers = [], totalSeller = 0, loadingRequests } = useSelector((state) => state.seller);
    ...
    useEffect(() => {
     ...
     const isLoading = loadingRequests === undefined ? !sellers : loadingRequests;
                ...
                <div className="w-full rounded-xl p-5 sm:p-6 bg-[#252A40]/70 backdrop-blur-md border border-gray-700/80 shadow-lg">
                    <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <label htmlFor="requestParPage" className="text-sm text-gray-400 whitespace-nowrap">Show:</label>
                            <select
                                id="requestParPage"
                    ...
                    <div className="relative overflow-x-auto rounded-lg border border-gray-700/50">
                        <table className="w-full text-sm text-left text-gray-300">
                            ...
                            <tbody className="divide-y divide-gray-700/50">
                                {isLoading ? (
                                     Array.from({ length: parPage }).map((_, i) => <SkeletonRequestRow key={i} />)
                                ) : sellers?.length > 0 ? (
                                    sellers.map((d, i) => (
                                        <tr key={d._id} className="hover:bg-[#303650]/40 transition-colors">
                                            <td className="py-4 px-4 sm:px-6 font-medium text-gray-400"> {(currentPage - 1) * parPage + i + 1} </td>
                                            <td className="py-4 px-4 sm:px-6 font-medium text-gray-200 whitespace-nowrap">{d.name || 'N/A'}</td>
                                            <td className="py-4 px-4 sm:px-6 text-gray-300 whitespace-nowrap">{d.email || 'N/A'}</td>
                                            <td className="py-4 px-4 sm:px-6"> <SellerStatusBadge status={d.payment} type="payment" /> </td>
                                            <td className="py-4 px-4 sm:px-6"> <SellerStatusBadge status={d.status} type="account" /> </td>
                                            <td className="py-4 px-4 sm:px-6 text-center"> <Link to={`/admin/dashboard/seller/details/${d._id}`} className="p-2 inline-flex items-center justify-center text-cyan-400 hover:text-cyan-300 bg-cyan-500/10 hover:bg-cyan-500/20 rounded-md transition-all duration-300" aria-label={`View details for ${d.name}`}> <FaEye size={14} /> </Link> </td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr> <td colSpan="6" className="py-8 px-4 sm:px-6 text-center text-gray-500 text-sm"> No pending seller requests found{searchValue ? ` matching "${searchValue}"` : ''}. </td> </tr>
                                )}
                            </tbody>
                        </table>
                    </div>
